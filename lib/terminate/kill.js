...
const setKillTimeout = async ({kill, signal, forceKillAfterDelay, killSignal, killResult, context, controller}) => {
	// When the `gracefulTimeout` option is used, the escalation to `SIGKILL` is handled
	// by `killOnGracefulTimeout()` instead, so it does not fire before `gracefulTimeout` expires
	if (signal === killSignal && killResult && !context.isGracefulTimeout) {
		killOnTimeout({
			kill,
			forceKillAfterDelay,
			context,
