...
const getErrorPrefix = ({
	...
	if (timedOut) {
		if (gracefulTimeout !== undefined) {
			return getGracefulTimeoutPrefix(timeout, gracefulTimeout, isForcefullyTerminated);
		}
...
const getGracefulTimeoutPrefix = (timeout, gracefulTimeout, isForcefullyTerminated) => isForcefullyTerminated
	? `Command timed out after ${timeout} milliseconds and was forcefully terminated with SIGKILL after the graceful timeout of ${gracefulTimeout} milliseconds`
	: `Command timed out after ${timeout} milliseconds and was gracefully terminated with SIGTERM within the graceful timeout of ${gracefulTimeout} milliseconds`;
