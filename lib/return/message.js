...
const getErrorPrefix = ({
	...
	if (timedOut) {
		if (gracefulTimeout !== undefined) {
			return getGracefulTimeoutPrefix({
				timeout,
				gracefulTimeout,
				isForcefullyTerminated,
				signal,
...
// Mention whether the subprocess exited gracefully after `SIGTERM` or had to be forcefully
// terminated with `SIGKILL`, so timeouts using `gracefulTimeout` can be told apart
const getGracefulTimeoutPrefix = ({timeout, gracefulTimeout, isForcefullyTerminated, signal, signalDescription}) => {
	if (isForcefullyTerminated) {
		return `Command timed out after ${timeout} milliseconds and was forcefully terminated with SIGKILL after a graceful timeout of ${gracefulTimeout} milliseconds`;
	}

	const signalSuffix = signal === undefined ? '' : ` with ${signal} (${signalDescription})`;
	return `Command timed out after ${timeout} milliseconds and was gracefully terminated${signalSuffix} within a graceful timeout of ${gracefulTimeout} milliseconds`;
};
